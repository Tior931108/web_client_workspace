...
document.querySelector('#btn3').addEventListener('click', (e) => {
    console.log('🍔🍔🍔', e);
    // console.log(document.querySelector('#btn3').addEventListener);
});
...
document.querySelector('#btn4').onclick = function(e) {
    ...
    // this용법6. 이벤트핸들러 안의 this는 이벤트 발생객체를 가리킨다.
    console.log(this);
...
/**
 * 제출버튼 클릭 -> submit 이벤트 -> submit이벤트 핸들러(유효성검사) 호출 ->제출
 * 
...
// blur 이벤트 핸글러 바인딩
// 주의!! 함수 연결이지 함수 호출()하면안됨!
confirmPassword.onblur = notEqualPasswords;

const requestSignup = () => {
    console.log('폼 제출 성공~ 😁');

    const enteredUsername = frm.username.value;
    const enteredPassword = frm.password.value;

    // signupFrm의 내용출력 / 초기화
    // honggd/1234 회원가입을 요청했습니다.
    alert(`${enteredUsername}/${enteredPassword} 회원가입을 요청했습니다.`);
